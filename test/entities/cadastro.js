...
module.exports = function (config) {
    ...
    _.forEach(
      ...
      function (classe, property) {
        if (
          ...
          throw new Error(
            "Classe '" + classe + "' deve ser informada"
          )
        }
      }.bind(this)
    )
  ...
  cadAtivo.ClassificaçãoCad.afterCreate(
    'ac',
    function (options, result) {
      if (this.quitado === 'Z') {
        assert(
  ...
  cadAtivo.ClassificaçãoCad.afterUpdate(
    'au',
    function (options, result) {
      assert(this.entity, 'this should be a instance in afterUpdate')
      assert(
  ...
  cadAtivo.ClassificaçãoCad.afterDelete(
    'ad',
    function (options, result) {
      if (this.quitado === 'Z') {
        assert(
