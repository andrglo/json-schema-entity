      ...
      methods: {
        ...
        hasMany: function (schemaName, schema, options) {
          ...
          publicAssociationMethods.map(function (name) {
            data.public[association.key][name] =
              association.methods[name]
          })
        ...
        hasOne: function (schemaName, schema) {
          ...
          data.public[association.key] = association.public
          publicAssociationMethods.map(function (name) {
            data.public[association.key][name] = association.methods[
              name
            ].bind(association.methods)
          })
...
function getForeignKey(table, properties) {
  ...
  _.forEach(properties, function (property, name) {
    var $ref =
      property.$ref || (property.schema && property.schema.$ref)
    if ($ref) {
...
function buildTable(data) {
  ...
  _.forEach(data.requestedProperties, function (property, name) {
    ...
    if (
      ...
      !(
        (data.properties[name].autoIncrement ||
          data.foreignKey === name) &&
        data.isAssociation
      )
    ) {
  ...
  _.forEach(data.associations, function (association) {
    association.data.foreignKey =
      ...
      getForeignKey(
        data.identity.name,
        association.data.requestedProperties
      )
  ...
  _.forEach(data.properties, function (property, name) {
    data.coerce.push({
      property: name,
      fn: data.adapter.getCoercionFunction(
        property.type,
        property.timezone
      )
    })
